...
Uses a separate token file from the Calendar service so that expanding Gmail
scopes never invalidates the Calendar token.
"""
...
_TOKEN_PATH: Path = _PROJECT_ROOT / "credentials" / "gmail_token.json"
...
def get_gmail_service(
    ...
    Args:
        ...
        token_path: Path where the OAuth2 token is stored after the first
            ...
            ``credentials/gmail_token.json`` in the project root.
        scopes: OAuth2 scopes to request.  Defaults to Gmail modify + send.

    Returns:
        A ``googleapiclient`` Resource object ready to make Gmail API calls.
